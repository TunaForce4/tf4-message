...
public interface ClientDeliveryService {
    ...
    @GetMapping("/deliveris/{date}")
    List<GetDeliveriesResponseDto> getListDelivery(@PathVariable String date);
    //업체 배송 담당자 조건으로 쿼리
    @GetMapping("/delivery-agents")
    List<GetDeliverymenResponseDto> getListDeliverymen(@RequestParam("q") String role);
